...
#aparser.add_argument('-med'     ,'--medrange'  ,dest='medrange',nargs='+',type=int,    default=[10000],help='mediator range')
    ...
    for dm in args1.dmrange:
        ...
        if not args1.resubmit:
            for f in parameterdir:
                if f.find('model') == -1:
                    continue
                os.system('cp -r %s/%s/%s models/%s_%s_%s' % (basedir,args1.carddir,f,f,tmpMed,dm))
                replace(args1.name,tmpMed,dm,rand,'models/%s_%s_%s' % (f,tmpMed,dm))
                os.chdir('models/%s_%s_%s' % (f,tmpMed,dm))
                os.system('python write_param_card.py')
                os.chdir('%s/%s_MG5_aMC_v2_2_2' % (basedir,args1.name))
                os.system('mkdir MG_%s_%s_%s' % (args1.name,tmpMed,dm))
                
            for f in parameterfiles:
                with open('MG_%s_%s_%s/%s' % (args1.name,tmpMed,dm,f), "wt") as fout: 
                    with open(basedir+'/'+args1.carddir+'/'+f        ,"rt") as fin: 
                        for line in fin:
                            ...
                            fout.write(tmpline)
            job_file = open('%s/%s_MG5_aMC_v2_2_2/MG_%s_%s_%s/integrate.sh' % (basedir,args1.name,args1.name,tmpMed,dm), "wt")
            job_file.write('#!/bin/bash\n')
            job_file.write('cp -r %s/%s_MG5_aMC_v2_2_2/MG_%s_%s_%s/  .     \n' % (basedir,args1.name,args1.name,tmpMed,dm))
            ...
            output  ='%s_%s_%s_tarball.tar.xz'                    % (args1.name,tmpMed,dm)
            ...
            job_file.close()
            os.chmod('%s/%s_MG5_aMC_v2_2_2/MG_%s_%s_%s/integrate.sh' % (basedir,args1.name,args1.name,tmpMed,dm),0777)
        print "Looking",'%s/%s_MG5_aMC_v2_2_2/MG_%s_%s_%s/integrate.sh' % (basedir,args1.name,args1.name,tmpMed,dm)
        if os.path.isfile('%s/%s_MG5_aMC_v2_2_2/MG_%s_%s_%s/integrate.sh' % (basedir,args1.name,args1.name,tmpMed,dm)):
            os.system('bsub -q  %s -R "rusage[mem=12000]" %s/%s_MG5_aMC_v2_2_2/MG_%s_%s_%s/integrate.sh' % (args1.queue,basedir,args1.name,args1.name,tmpMed,dm))
        output     ='%s_%s_%s_tarball.tar.xz'                    % (args1.name,tmpMed,dm)
        lepoutput  ='%s_%s_%s_lep_tarball.tar.xz'                % (args1.name,tmpMed,dm)
